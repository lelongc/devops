# 🐙 Bài 02: Thành Thạo Lệnh Git CLI Căn Bản & Chuẩn Conventional Commits - Git Cho DevOps & Project Manager

> 💡 **Bản chất 1 câu**: **Bài 02: Thành Thạo Lệnh Git CLI Căn Bản & Chuẩn Conventional Commits** giải quyết bài toán quản lý phiên bản mã nguồn phân tán (Distributed Version Control System - DVCS), quản lý luồng phát triển phần mềm, quy chuẩn giao tiếp nhóm và tự động hóa chuỗi cung ứng phần mềm CI/CD / GitOps.  
> 🎯 **Trọng tâm thực chiến DevOps & Project Manager**: Các lệnh CLI căn bản: `git init`, `clone`, `add`, `commit`, `status`, `diff`, `log`, `show`, `restore`, `rm`, `mv` & Quy chuẩn viết Commit Message tiếng Anh/Việt chuẩn Conventional Commits (feat, fix, docs, refactor, ci, chore).

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 02: Thành Thạo Lệnh Git CLI Căn Bản & Chuẩn Conventional Commits** giống như việc điều hành một hệ thống ghi hình camera nhiều góc quay trong một dự án điện ảnh Hollywood:
...
- **Git (Bài 02: Thành Thạo Lệnh Git CLI Căn Bản & Chuẩn Conventional Commits)** đóng vai trò là cỗ máy thời gian ghi lại chính xác từng khung hình (Commits), cho phép tua lại (Reset/Revert), so sánh khác biệt (Diff), và ghép nhiều góc quay của các diễn viên lại thành một bộ phim hoàn chỉnh (Merge) mà không bao giờ lo mất mát dữ liệu.
...
```mermaid
graph LR
    Feat[feat: add user login API] --> Commit[Conventional Commit Standard]
    Fix[fix: resolve null pointer crash] --> Commit
    Docs[docs: update API endpoints README] --> Commit
```
