# 🐙 Bài 01: Kiến Trúc Bên Dưới Git Engine & Cấu Trúc Đối Tượng DAG - Git Cho DevOps & Project Manager

> 💡 **Bản chất 1 câu**: **Bài 01: Kiến Trúc Bên Dưới Git Engine & Cấu Trúc Đối Tượng DAG** giải quyết bài toán quản lý phiên bản mã nguồn phân tán (Distributed Version Control System - DVCS), quản lý luồng phát triển phần mềm, quy chuẩn giao tiếp nhóm và tự động hóa chuỗi cung ứng phần mềm CI/CD / GitOps.  
> 🎯 **Trọng tâm thực chiến DevOps & Project Manager**: Cơ chế lưu trữ bên dưới Git: Blob objects, Tree objects, Commit objects, Annotated Tags, Cấu trúc thư mục `.git`, Đồ thị hướng không chu trình DAG (Directed Acyclic Graph) & Working Tree vs Index (Staging Area) vs Repository.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 01: Kiến Trúc Bên Dưới Git Engine & Cấu Trúc Đối Tượng DAG** giống như việc điều hành một hệ thống ghi hình camera nhiều góc quay trong một dự án điện ảnh Hollywood:
...
- **Git (Bài 01: Kiến Trúc Bên Dưới Git Engine & Cấu Trúc Đối Tượng DAG)** đóng vai trò là cỗ máy thời gian ghi lại chính xác từng khung hình (Commits), cho phép tua lại (Reset/Revert), so sánh khác biệt (Diff), và ghép nhiều góc quay của các diễn viên lại thành một bộ phim hoàn chỉnh (Merge) mà không bao giờ lo mất mát dữ liệu.
...
![Git Architecture](https://upload.wikimedia.org/wikipedia/commons/e/e0/Git-logo.svg)

```mermaid
graph TD
    WorkingTree[Working Directory - Local Edit] -->|git add| Staging[Staging Area / Index]
    Staging -->|git commit| LocalRepo[Local Repository .git - DAG Commits]
