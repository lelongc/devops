# 🐍 14.Async_Programming_Asyncio_and_Concurrency: Lập Trình Bất Đồng Bộ Asyncio & Concurrency - Python Chuyên Sâu Cho DevOps

> 💡 **Bản chất 1 câu**: Lập trình đa nhiệm và bất đồng bộ trong Python: Multithreading (`threading`, `ThreadPoolExecutor`), Multiprocessing (`multiprocessing`), và Async I/O (`asyncio`, `aiohttp`).  
> 🎯 **Trọng tâm thực chiến DevOps**: Nắm vững GIL (Global Interpreter Lock), CPU-bound vs I/O-bound tasks, `async/await`, `asyncio.gather()`, chạy song song 1,000 HTTP API requests trong 2 giây.

---

```mermaid
graph TD
    AsyncIO[Asyncio Event Loop] -->|async/await| Task1[HTTP Req 1]
    AsyncIO -->|async/await| Task2[HTTP Req 2]
    AsyncIO -->|async/await| Task3[HTTP Req 3]
```
...
### 2.1 So Sánh Multithreading, Multiprocessing & Asyncio (OBJ 14.1)
...
graph TD
    ...
    Concurrency --> AsyncIO[Asyncio: Single-thread Event Loop async/await - Siêu tối ưu I/O Network]
```
