# ☸️ Bài 18: Quản Lý Tài Nguyên - Resource Requests/Limits, ResourceQuota & LimitRanges - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 18: Quản Lý Tài Nguyên - Resource Requests/Limits, ResourceQuota & LimitRanges** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Quản lý tài nguyên cụm: CPU/Memory Requests & Limits, ResourceQuota (Namespace limit), LimitRange (Default container limits), QoS Classes (Guaranteed, Burstable, BestEffort) & OOM Killer prevention.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 18: Quản Lý Tài Nguyên - Resource Requests/Limits, ResourceQuota & LimitRanges** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
- **Kubernetes (Bài 18: Quản Lý Tài Nguyên - Resource Requests/Limits, ResourceQuota & LimitRanges)** đóng vai trò là Nhạc trưởng (Conductor) thiên tài. Nhạc trưởng không trực tiếp chơi nhạc cụ nhưng biết chính xác từng nhạc công đang làm gì, khi nào cần tăng/giảm nhịp độ (Autoscaling), và khi có một nhạc công bị ngất xỉu (Pod Crash), Nhạc trưởng lập tức gọi nhạc công dự phòng lên thay thế ngay lập tức trong miligiây mà bản nhạc không hề bị ngắt quãng.
...
```mermaid
graph TD
    Pod[Pod Spec Requests/Limits] --> QoSClass{QoS Evaluation}
    QoSClass -->|Requests == Limits| Guaranteed[Guaranteed QoS - High Priority]
    QoSClass -->|Requests < Limits| Burstable[Burstable QoS - Med Priority]
    QoSClass -->|No Requests/Limits| BestEffort[BestEffort QoS - First to be Killed on OOM]
```
