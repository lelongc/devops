# ☸️ Bài 11: Quản Lý Điều Hướng Ingress & Tự Động Cấp Phát SSL/TLS Cert-Manager - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 11: Quản Lý Điều Hướng Ingress & Tự Động Cấp Phát SSL/TLS Cert-Manager** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Ingress Router: Nginx Ingress Controller, Path-based & Host-based Routing, Tự động xin & gia hạn chứng chỉ HTTPS SSL/TLS với `cert-manager` & Let's Encrypt CA.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 11: Quản Lý Điều Hướng Ingress & Tự Động Cấp Phát SSL/TLS Cert-Manager** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
- **Kubernetes (Bài 11: Quản Lý Điều Hướng Ingress & Tự Động Cấp Phát SSL/TLS Cert-Manager)** đóng vai trò là Nhạc trưởng (Conductor) thiên tài. Nhạc trưởng không trực tiếp chơi nhạc cụ nhưng biết chính xác từng nhạc công đang làm gì, khi nào cần tăng/giảm nhịp độ (Autoscaling), và khi có một nhạc công bị ngất xỉu (Pod Crash), Nhạc trưởng lập tức gọi nhạc công dự phòng lên thay thế ngay lập tức trong miligiây mà bản nhạc không hề bị ngắt quãng.
...
```mermaid
graph TD
    HTTPS[HTTPS User Request domain.com] --> Ingress[Nginx Ingress Controller]
    CertManager[cert-manager Operator] -->|ACME HTTP-01| LetsEncrypt[Let's Encrypt CA]
    CertManager -->|TLS Secret| Ingress
