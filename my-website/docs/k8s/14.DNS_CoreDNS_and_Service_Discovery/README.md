# ☸️ Bài 14: Cơ Chế Phân Giải Tên Miền CoreDNS & Service Discovery - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 14: Cơ Chế Phân Giải Tên Miền CoreDNS & Service Discovery** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Phân giải tên miền K8s: CoreDNS architecture, Cấu trúc FQDN (`service.namespace.svc.cluster.local`), `dnsPolicy`, `dnsConfig`, `autopath` & Troubleshooting lỗi DNS timeout.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 14: Cơ Chế Phân Giải Tên Miền CoreDNS & Service Discovery** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
- **Kubernetes (Bài 14: Cơ Chế Phân Giải Tên Miền CoreDNS & Service Discovery)** đóng vai trò là Nhạc trưởng (Conductor) thiên tài. Nhạc trưởng không trực tiếp chơi nhạc cụ nhưng biết chính xác từng nhạc công đang làm gì, khi nào cần tăng/giảm nhịp độ (Autoscaling), và khi có một nhạc công bị ngất xỉu (Pod Crash), Nhạc trưởng lập tức gọi nhạc công dự phòng lên thay thế ngay lập tức trong miligiây mà bản nhạc không hề bị ngắt quãng.
...
```mermaid
graph TD
    Pod[Pod Query: db-service] --> CoreDNS[CoreDNS Pods]
    CoreDNS -->|Resolves IP| ClusterIP[10.96.150.40]
```
