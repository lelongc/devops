# ☸️ Bài 02: Dựng Cụm K8s Khả Dụng Cao (HA) Bằng Kubeadm & Gia Hạn Chứng Chỉ SSL - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 02: Dựng Cụm K8s Khả Dụng Cao (HA) Bằng Kubeadm & Gia Hạn Chứng Chỉ SSL** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Khởi tạo cụm K8s từ đầu: `kubeadm init`, `kubeadm join`, CNI Flannel/Calico, HA Control Plane với HAProxy + Keepalived & Gia hạn chứng chỉ SSL `kubeadm certs renew`.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 02: Dựng Cụm K8s Khả Dụng Cao (HA) Bằng Kubeadm & Gia Hạn Chứng Chỉ SSL** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
- **Kubernetes (Bài 02: Dựng Cụm K8s Khả Dụng Cao (HA) Bằng Kubeadm & Gia Hạn Chứng Chỉ SSL)** đóng vai trò là Nhạc trưởng (Conductor) thiên tài. Nhạc trưởng không trực tiếp chơi nhạc cụ nhưng biết chính xác từng nhạc công đang làm gì, khi nào cần tăng/giảm nhịp độ (Autoscaling), và khi có một nhạc công bị ngất xỉu (Pod Crash), Nhạc trưởng lập tức gọi nhạc công dự phòng lên thay thế ngay lập tức trong miligiây mà bản nhạc không hề bị ngắt quãng.
...
```mermaid
graph TD
    VIP[Virtual IP Keepalived] --> HAProxy[HAProxy Load Balancer :6443]
    HAProxy --> Master1[Master Node 1: kube-apiserver]
    HAProxy --> Master2[Master Node 2: kube-apiserver]
    HAProxy --> Master3[Master Node 3: kube-apiserver]
```
