# ☸️ Bài 05: Quản Lý Deployments, ReplicaSets & Các Chiến Lược Rollout - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 05: Quản Lý Deployments, ReplicaSets & Các Chiến Lược Rollout** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Quản lý Deployments: ReplicaSets, Chiến lược RollingUpdate (`maxSurge`, `maxUnavailable`), Recreate, Rollout Status, Rollout History, Rollout Undo / Rollback zero downtime.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 05: Quản Lý Deployments, ReplicaSets & Các Chiến Lược Rollout** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
- **Kubernetes (Bài 05: Quản Lý Deployments, ReplicaSets & Các Chiến Lược Rollout)** đóng vai trò là Nhạc trưởng (Conductor) thiên tài. Nhạc trưởng không trực tiếp chơi nhạc cụ nhưng biết chính xác từng nhạc công đang làm gì, khi nào cần tăng/giảm nhịp độ (Autoscaling), và khi có một nhạc công bị ngất xỉu (Pod Crash), Nhạc trưởng lập tức gọi nhạc công dự phòng lên thay thế ngay lập tức trong miligiây mà bản nhạc không hề bị ngắt quãng.
...
```mermaid
graph TD
    Deployment[Deployment v2] -->|Create| RS_New[ReplicaSet v2: Pod 1, Pod 2, Pod 3]
    Deployment -->|Scale Down| RS_Old[ReplicaSet v1: Pod Old - Terminated]
```
