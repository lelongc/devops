# ☸️ Bài 16: Phân Quyền Bảo Mật RBAC, ServiceAccounts & OIDC Authentication - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 16: Phân Quyền Bảo Mật RBAC, ServiceAccounts & OIDC Authentication** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Quản lý truy cập: Authentication (X.509 Certs, OIDC), ServiceAccount, RBAC (Role, RoleBinding, ClusterRole, ClusterRoleBinding), `kubectl auth can-i` & IRSA AWS IAM for Service Accounts.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 16: Phân Quyền Bảo Mật RBAC, ServiceAccounts & OIDC Authentication** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
```mermaid
graph TD
    Subject[ServiceAccount / User] --> RoleBinding[RoleBinding / ClusterRoleBinding]
    RoleBinding --> Role[Role / ClusterRole: verbs: get, list, create]
    Role --> Resource[Kubernetes API Resource: Pods, Deployments]
```
