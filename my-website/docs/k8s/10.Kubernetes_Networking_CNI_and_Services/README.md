# ☸️ Bài 10: Chuyên Sâu Mạng K8s, CNI Plugins & Các Loại Services - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 10: Chuyên Sâu Mạng K8s, CNI Plugins & Các Loại Services** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Mạng K8s: CNI Plugins (Flannel, Calico, Cilium eBPF), IP-per-Pod model, Service Types (ClusterIP, NodePort, LoadBalancer, ExternalName), Headless Service & EndpointSlices.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 10: Chuyên Sâu Mạng K8s, CNI Plugins & Các Loại Services** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
```mermaid
graph TD
    Client[External Client] --> LB[LoadBalancer Service IP]
    LB --> KubeProxy[kube-proxy eBPF / iptables]
    KubeProxy --> Pod1[Pod 1: 10.244.1.15:8080]
    KubeProxy --> Pod2[Pod 2: 10.244.2.20:8080]
```
