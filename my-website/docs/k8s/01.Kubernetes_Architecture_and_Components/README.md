# ☸️ Bài 01: Kiến Trúc Cụm Kubernetes & Các Thành Phần Control Plane / Worker Node - Kubernetes Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 01: Kiến Trúc Cụm Kubernetes & Các Thành Phần Control Plane / Worker Node** giải quyết bài toán cốt lõi trong việc tự động hóa điều phối (Orchestration), vận hành quy mô lớn, đảm bảo tính sẵn sàng cao (High Availability 99.999%) và tự phục hồi (Self-healing) cho các ứng dụng Containerized chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps & Kubernetes Architect**: Bản chất Kubernetes Architecture: Control Plane (etcd, kube-apiserver, kube-scheduler, kube-controller-manager), Worker Node (kubelet, kube-proxy, Container Runtime CRI containerd) & Luồng xử lý khi tạo Pod.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 01: Kiến Trúc Cụm Kubernetes & Các Thành Phần Control Plane / Worker Node** giống như việc điều hành một dàn nhạc giao hưởng khổng lồ quốc tế:
...
- **Kubernetes (Bài 01: Kiến Trúc Cụm Kubernetes & Các Thành Phần Control Plane / Worker Node)** đóng vai trò là Nhạc trưởng (Conductor) thiên tài. Nhạc trưởng không trực tiếp chơi nhạc cụ nhưng biết chính xác từng nhạc công đang làm gì, khi nào cần tăng/giảm nhịp độ (Autoscaling), và khi có một nhạc công bị ngất xỉu (Pod Crash), Nhạc trưởng lập tức gọi nhạc công dự phòng lên thay thế ngay lập tức trong miligiây mà bản nhạc không hề bị ngắt quãng.

---

## 2. 📚 Lý Thuyết Chuyên Sâu & Bản Chất Hệ Thống (Under The Hood Architecture)

![Kubernetes Architecture](https://upload.wikimedia.org/wikipedia/commons/3/39/Kubernetes_logo_without_workmark.svg)

```mermaid
graph TD
    Client[kubectl CLI / User] -->|HTTPS REST API| APIServer[kube-apiserver]
    APIServer <--> ETCD[(etcd Key-Value Store)]
    APIServer <--> Scheduler[kube-scheduler]
    APIServer <--> Controller[kube-controller-manager]
    APIServer -->|gRPC CRI| Kubelet1[Worker Node 1: kubelet]
    APIServer -->|gRPC CRI| Kubelet2[Worker Node 2: kubelet]
    Kubelet1 --> Containerd1[containerd -> Pods]
    Kubelet2 --> Containerd2[containerd -> Pods]
```
