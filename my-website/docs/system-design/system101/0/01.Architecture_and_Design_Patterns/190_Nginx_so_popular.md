# 🏛 Why is Nginx so popular? - System Design 101 Chuyên Sâu Toàn Tập

> 💡 **Bản chất 1 câu**: **Why is Nginx so popular?** là khái niệm kiến trúc quan trọng trong thiết kế hệ thống quy mô lớn (High Scale Distributed Systems), giúp xử lý tải hàng triệu truy vấn đồng thời, tối ưu độ trễ và đảm bảo tính sẵn sàng 99.999%.  
...
Hãy tưởng tượng **Why is Nginx so popular?** giống như việc quy hoạch hạ tầng giao thông tại một đô thị siêu lớn:
- **Client / User**: Hàng triệu phương tiện giao thông di chuyển cùng lúc.
- **Thành phần kiến trúc (Why is Nginx so popular?)**: Hệ thống đèn giao thông thông minh, cầu vượt, đường cao tốc phân luồng và trạm thu phí tự động.
...
### 🖼 Sơ Đồ Đồ Họa Gốc Trích Xuất Từ Sách System Design 101 (Page 325):

![Sơ đồ đồ họa gốc Why is Nginx so popular?](../images/page_325_img_1.png)
...
Khái niệm **Why is Nginx so popular?** giải quyết bài toán cốt lõi trong thiết kế hệ thống phân tán quy mô lớn. 
...
| :--- | :--- | :--- | :--- |
| **`Why is Nginx so popular?`** | `System Architecture` | Giải pháp kiến trúc quy mô lớn xử lý High Concurrency | `Triển khai Microservices, Cloud Native & Distributed Systems` |
...
> **Q: Khi áp dụng kiến trúc Why is Nginx so popular?, bài toán Đánh đổi (Trade-offs) lớn nhất mà kỹ sư phải giải quyết là gì?**  
...
- [x] Hiểu rõ nguyên lý hoạt động cốt lõi của **Why is Nginx so popular?** và vị trí của nó trong sơ đồ tổng thể.
