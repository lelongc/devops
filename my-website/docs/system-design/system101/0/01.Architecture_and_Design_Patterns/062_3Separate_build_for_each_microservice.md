# 🏛 3.​Separate build for each microservice - System Design 101 Chuyên Sâu Toàn Tập

> 💡 **Bản chất 1 câu**: **3.​Separate build for each microservice** là khái niệm kiến trúc quan trọng trong thiết kế hệ thống quy mô lớn (High Scale Distributed Systems), giúp xử lý tải hàng triệu truy vấn đồng thời, tối ưu độ trễ và đảm bảo tính sẵn sàng 99.999%.  
...
Hãy tưởng tượng **3.​Separate build for each microservice** giống như việc quy hoạch hạ tầng giao thông tại một đô thị siêu lớn:
- **Client / User**: Hàng triệu phương tiện giao thông di chuyển cùng lúc.
- **Thành phần kiến trúc (3.​Separate build for each microservice)**: Hệ thống đèn giao thông thông minh, cầu vượt, đường cao tốc phân luồng và trạm thu phí tự động.
...
![Sơ đồ đồ họa gốc 3.​Separate build for each microservice](../images/page_102_full.png)
...
Khái niệm **3.​Separate build for each microservice** giải quyết bài toán cốt lõi trong thiết kế hệ thống phân tán quy mô lớn. 
...
| :--- | :--- | :--- | :--- |
| **`3.​Separate build for each microservice`** | `System Architecture` | Giải pháp kiến trúc quy mô lớn xử lý High Concurrency | `Triển khai Microservices, Cloud Native & Distributed Systems` |
...
> **Q: Khi áp dụng kiến trúc 3.​Separate build for each microservice, bài toán Đánh đổi (Trade-offs) lớn nhất mà kỹ sư phải giải quyết là gì?**  
...
- [x] Hiểu rõ nguyên lý hoạt động cốt lõi của **3.​Separate build for each microservice** và vị trí của nó trong sơ đồ tổng thể.
