# 🏛 Reverse proxy vs. API gateway vs. load balancer​ - System Design 101 Chuyên Sâu Toàn Tập

> 💡 **Bản chất 1 câu**: **Reverse proxy vs. API gateway vs. load balancer​** là khái niệm kiến trúc quan trọng trong thiết kế hệ thống quy mô lớn (High Scale Distributed Systems), giúp xử lý tải hàng triệu truy vấn đồng thời, tối ưu độ trễ và đảm bảo tính sẵn sàng 99.999%.  
...
Hãy tưởng tượng **Reverse proxy vs. API gateway vs. load balancer​** giống như việc quy hoạch hạ tầng giao thông tại một đô thị siêu lớn:
- **Client / User**: Hàng triệu phương tiện giao thông di chuyển cùng lúc.
- **Thành phần kiến trúc (Reverse proxy vs. API gateway vs. load balancer​)**: Hệ thống đèn giao thông thông minh, cầu vượt, đường cao tốc phân luồng và trạm thu phí tự động.
...
### 🖼 Sơ Đồ Đồ Họa Gốc Trích Xuất Từ Sách System Design 101 (Page 139):

![Sơ đồ đồ họa gốc Reverse proxy vs. API gateway vs. load balancer​](../images/page_139_full.png)
...
Khái niệm **Reverse proxy vs. API gateway vs. load balancer​** giải quyết bài toán cốt lõi trong thiết kế hệ thống phân tán quy mô lớn. 
...
| :--- | :--- | :--- | :--- |
| **`Reverse proxy vs. API gateway vs. load balancer​`** | `System Architecture` | Giải pháp kiến trúc quy mô lớn xử lý High Concurrency | `Triển khai Microservices, Cloud Native & Distributed Systems` |
...
> **Q: Khi áp dụng kiến trúc Reverse proxy vs. API gateway vs. load balancer​, bài toán Đánh đổi (Trade-offs) lớn nhất mà kỹ sư phải giải quyết là gì?**  
...
- [x] Hiểu rõ nguyên lý hoạt động cốt lõi của **Reverse proxy vs. API gateway vs. load balancer​** và vị trí của nó trong sơ đồ tổng thể.
