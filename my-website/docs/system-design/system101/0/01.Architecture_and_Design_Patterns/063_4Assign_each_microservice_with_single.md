# 🏛 4.​Assign each microservice with a single responsibility - System Design 101 Chuyên Sâu Toàn Tập

> 💡 **Bản chất 1 câu**: **4.​Assign each microservice with a single responsibility** là khái niệm kiến trúc quan trọng trong thiết kế hệ thống quy mô lớn (High Scale Distributed Systems), giúp xử lý tải hàng triệu truy vấn đồng thời, tối ưu độ trễ và đảm bảo tính sẵn sàng 99.999%.  
...
Hãy tưởng tượng **4.​Assign each microservice with a single responsibility** giống như việc quy hoạch hạ tầng giao thông tại một đô thị siêu lớn:
- **Client / User**: Hàng triệu phương tiện giao thông di chuyển cùng lúc.
- **Thành phần kiến trúc (4.​Assign each microservice with a single responsibility)**: Hệ thống đèn giao thông thông minh, cầu vượt, đường cao tốc phân luồng và trạm thu phí tự động.
...
![Sơ đồ đồ họa gốc 4.​Assign each microservice with a single responsibility](../images/page_102_full.png)
...
Khái niệm **4.​Assign each microservice with a single responsibility** giải quyết bài toán cốt lõi trong thiết kế hệ thống phân tán quy mô lớn. 
...
| :--- | :--- | :--- | :--- |
| **`4.​Assign each microservice with a single responsibility`** | `System Architecture` | Giải pháp kiến trúc quy mô lớn xử lý High Concurrency | `Triển khai Microservices, Cloud Native & Distributed Systems` |
...
> **Q: Khi áp dụng kiến trúc 4.​Assign each microservice with a single responsibility, bài toán Đánh đổi (Trade-offs) lớn nhất mà kỹ sư phải giải quyết là gì?**  
...
- [x] Hiểu rõ nguyên lý hoạt động cốt lõi của **4.​Assign each microservice with a single responsibility** và vị trí của nó trong sơ đồ tổng thể.
