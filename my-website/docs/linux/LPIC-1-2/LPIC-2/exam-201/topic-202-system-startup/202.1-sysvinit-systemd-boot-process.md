...
```mermaid
graph TD
    Boot[Kernel Booted] --> Systemd[Systemd PID 1]
    Systemd --> SysInit[sysinit.target - Mount /proc, /sys, SWAP]
    SysInit --> Basic[basic.target - Start Basic Hardware Services]
    Basic --> MultiUser[multi-user.target - Start Networking & Server Daemons]
    MultiUser --> Graphical[graphical.target - Start Display Manager GUI]
```
