# 🐳 Bài 01: Kiến Trúc Docker Engine, Kernel Namespaces & Cgroups - Docker Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 01: Kiến Trúc Docker Engine, Kernel Namespaces & Cgroups** giải quyết bài toán đóng gói, cô lập và tự động hóa vận hành ứng dụng trong môi trường Containerization chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps**: Bản chất Containerization: Linux Kernel Namespaces (PID, NET, MNT, IPC, UTS, USER), Control Groups (cgroups v1/v2), OverlayFS, Copy-on-Write (CoW), Docker Daemon, Containerd & runC.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 01: Kiến Trúc Docker Engine, Kernel Namespaces & Cgroups** giống như ngành vận tải biển quốc tế trước và sau khi có thùng Container tiêu chuẩn ISO:
...
![Docker Architecture Diagram](https://upload.wikimedia.org/wikipedia/commons/7/79/Docker_(container_engine)_logo.png)

```mermaid
graph TD
    Client[Docker CLI / API] --> Daemon[Docker Daemon dockerd]
    Daemon --> Containerd[containerd High-Level Runtime]
    Containerd --> RunC[runC OCI Low-Level Runtime]
    RunC --> Kernel[Linux Kernel: Namespaces + cgroups + OverlayFS]
```
