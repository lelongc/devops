# 🐳 Bài 02: Thành Thạo Docker CLI & Vòng Đời Container - Docker Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 02: Thành Thạo Docker CLI & Vòng Đời Container** giải quyết bài toán đóng gói, cô lập và tự động hóa vận hành ứng dụng trong môi trường Containerization chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps**: Quản lý vòng đời Container: `docker run`, `exec`, `stop`, `start`, `restart`, `rm`, `logs`, `inspect`, `stats`, `commit` & Vòng đời 5 trạng thái Created, Running, Paused, Stopped, Dead.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 02: Thành Thạo Docker CLI & Vòng Đời Container** giống như ngành vận tải biển quốc tế trước và sau khi có thùng Container tiêu chuẩn ISO:
...
```mermaid
graph LR
    Image[Docker Image] -->|docker run| Running[Running Container]
    Running -->|docker pause| Paused[Paused Container]
    Paused -->|docker unpause| Running
    Running -->|docker stop| Stopped[Stopped Container]
    Stopped -->|docker start| Running
    Stopped -->|docker rm| Destroyed[Container Deleted]
```
