# 🐳 Bài 17: Giám Sát Tài Nguyên (CPU/RAM Limits) & Quản Lý Log Container - Docker Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 17: Giám Sát Tài Nguyên (CPU/RAM Limits) & Quản Lý Log Container** giải quyết bài toán đóng gói, cô lập và tự động hóa vận hành ứng dụng trong môi trường Containerization chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps**: Quản lý log & tài nguyên Container: Logging Drivers (json-file, syslogs, fluentd, loki), Giới hạn tài nguyên RAM/CPU (`--memory=1g`, `--cpus=1.5`), cgroup OOM Killer & `docker stats`.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 17: Giám Sát Tài Nguyên (CPU/RAM Limits) & Quản Lý Log Container** giống như ngành vận tải biển quốc tế trước và sau khi có thùng Container tiêu chuẩn ISO:
...
```mermaid
graph TD
    ContainerLogs[Container STDOUT / STDERR] --> LogDriver[Docker Logging Driver]
    LogDriver --> Loki[Grafana Loki / Fluentd] --> Grafana[Grafana Dashboard]
```
