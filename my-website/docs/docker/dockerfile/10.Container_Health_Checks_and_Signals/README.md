# 🐳 Bài 10: Giám Sát Sức Khỏe Container (HEALTHCHECK) & Graceful Shutdown - Docker Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 10: Giám Sát Sức Khỏe Container (HEALTHCHECK) & Graceful Shutdown** giải quyết bài toán đóng gói, cô lập và tự động hóa vận hành ứng dụng trong môi trường Containerization chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps**: Cấu hình chỉ thị `HEALTHCHECK` (`--interval`, `--timeout`, `--retries`, `--start-period`), Xử lý ngắt tín hiệu `SIGTERM` / `SIGINT` cho ứng dụng thoát an toàn không mất dữ liệu.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 10: Giám Sát Sức Khỏe Container (HEALTHCHECK) & Graceful Shutdown** giống như ngành vận tải biển quốc tế trước và sau khi có thùng Container tiêu chuẩn ISO:
...
```mermaid
graph TD
    DockerDaemon[Docker Engine] -->|HEALTHCHECK CMD curl http://localhost:8080/health| ContainerStatus{Status Response}
    ContainerStatus -->|200 OK| Healthy[Status: Healthy]
    ContainerStatus -->|Timeout / 500 Error| Unhealthy[Status: Unhealthy -> Auto Restart]
```
