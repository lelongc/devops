# 🐳 Bài 06: Thành Thạo Chỉ Thị Dockerfile (FROM, RUN, CMD vs ENTRYPOINT) - Docker Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 06: Thành Thạo Chỉ Thị Dockerfile (FROM, RUN, CMD vs ENTRYPOINT)** giải quyết bài toán đóng gói, cô lập và tự động hóa vận hành ứng dụng trong môi trường Containerization chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps**: Cú pháp Dockerfile chuẩn mực: `FROM`, `RUN`, `CMD` vs `ENTRYPOINT` (Exec Form vs Shell Form), `COPY` vs `ADD`, `WORKDIR`, `ENV`, `ARG`, `EXPOSE`, `USER`, `VOLUME`, `STOPSIGNAL`.

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 06: Thành Thạo Chỉ Thị Dockerfile (FROM, RUN, CMD vs ENTRYPOINT)** giống như ngành vận tải biển quốc tế trước và sau khi có thùng Container tiêu chuẩn ISO:
...
```mermaid
graph TD
    BaseImage[FROM node:18-alpine] --> WorkDir[WORKDIR /app]
    WorkDir --> CopyDeps[COPY package*.json ./]
    CopyDeps --> RunInstall[RUN npm ci]
    RunInstall --> CopyApp[COPY . .]
    CopyApp --> Entry[ENTRYPOINT node server.js]
```
