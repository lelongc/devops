# 🐳 Bài 05: Phân Tích Chuyên Sâu Mạng Trong Docker (Bridge, Host, Overlay & Macvlan) - Docker Cho DevOps Chuyên Sâu

> 💡 **Bản chất 1 câu**: **Bài 05: Phân Tích Chuyên Sâu Mạng Trong Docker (Bridge, Host, Overlay & Macvlan)** giải quyết bài toán đóng gói, cô lập và tự động hóa vận hành ứng dụng trong môi trường Containerization chuẩn Cloud Native.  
> 🎯 **Trọng tâm thực chiến DevOps**: Các chế độ mạng Docker: Bridge Driver (Default docker0), Host Driver, None Driver, Overlay Driver (Swarm), Macvlan Driver, DNS Resolution nội bộ Container & Port Mapping iptables (`-p 8080:80`).

---

## 1. 🧠 Hình Hình Dung Nhanh Cho Người Mới (Intuitive Mindset)

Hãy tưởng tượng **Bài 05: Phân Tích Chuyên Sâu Mạng Trong Docker (Bridge, Host, Overlay & Macvlan)** giống như ngành vận tải biển quốc tế trước và sau khi có thùng Container tiêu chuẩn ISO:
...
```mermaid
graph TD
    HostPort[Host Port :8080] -->|iptables NAT rule| BridgeNet[Docker Bridge Network docker0]
    BridgeNet --> ContainerPort[Container IP 172.17.0.2 :80]
```
